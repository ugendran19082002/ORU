  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {/* Stats Grid */}
        <View style={[styles.statsGrid, { gap: isDesktop ? 20 : 10 }]}>
          {STATS.map((stat, i) => (
            <StatCard key={i} stat={stat} isDesktop={isDesktop} />
          ))}
        </View>

        <View style={{ flexDirection: isDesktop ? 'row' : 'column', gap: 32, marginTop: 12 }}>
            ...
            <View style={{ flex: isDesktop ? 2 : 1 }}>
                <View style={styles.sectionHeader}>
                    ...
                    <View style={styles.verifCountBadge}>
                        <Text style={styles.verifCountText}>{pendingShops.length} Pending</Text>
                    </View>
                </View>
                ...
                <View style={styles.verifCard}>
                    {loading ? (
                        ...
                        pendingShops.slice(0, 5).map((item, index) => (
                            <View key={item.id}>
                                <TouchableOpacity style={styles.verifRow} onPress={() => router.push(`/admin/shops/${item.id}`)}>
                                    <View style={styles.verifIcon}>
                                        <Ionicons name="business" size={24} color="#005d90" />
                                    </View>
                                    <View style={{ flex: 1 }}>
                                        <Text style={styles.verifShop}>{item.name}</Text>
                                        <Text style={styles.verifReason} numberOfLines={1}>{item.shop_type} • {item.city || 'Location Pending'}</Text>
