...
export default function ShopLocationScreen() {
  ...
  const [address, setAddress] = useState('Locating store...');
    ...
    (async () => {
      try {
        ...
        if (err.response?.status === 404) {
          ...
          console.error('[Shop Location] ID Resolution Error:', err);
        }
      } finally {
      ...
      try {
        ...
        if (status !== 'granted') {
          Toast.show({
            ...
            text2: 'We need location access to pin your shop correctly.'
          });
          setLocating(false);
          return;
        }
  ...
  const handleConfirm = async () => {
    if (!shopId) {
        Toast.show({
            type: 'error',
            text1: 'Error',
            text2: 'Shop context lost. Please restart onboarding.'
        });
        return;
    }
    ...
    try {
      ...
      const res = await onboardingApi.completeShopStep('set_location', shopId, {
      ...
      if (res.status === 1) {
        Toast.show({
            ...
            text2: 'Business address updated successfully.'
        });
        router.replace('/onboarding/shop');
      }
    } catch (error: any) {
      console.error('[Onboarding] Shop Location Save Error:', error);
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: 'Failed to save shop location.'
      });
    } finally {
      setLoading(false);
    }
        ...
        <View style={styles.header}>
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16, gap: 12 }}>
                ...
                <View style={[styles.stepContainer, { marginBottom: 0, flex: 1 }]}>
                    ...
                    <View style={[styles.stepDot, styles.stepDotActive, { backgroundColor: '#134e4a' }]} />
                </View>
            </View>
            <Text style={styles.title}>Shop Location</Text>
            <Text style={styles.subtitle}>Pin your business address for customers</Text>
        </View>
